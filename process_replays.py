...
def parse_showdown_replay(log_text, replay_id="unknown"):
    ...
    def find_slot_id(player, identifier_name, species_from_details):
        ...
        if identifier_name and player in nickname_to_slot_id and identifier_name in nickname_to_slot_id[player]:
            ...
            if found_slot_id in current_state.get(player, {}):
                # Check if current species matches (accounting for forme changes)
                current_slot_species = current_state[player][found_slot_id].get('species')
                # Allow match if either exact match OR both normalize to same base
                ...
                if initial_species == normalized_species_lookup:
                     return found_slot_id
                # Try soft match on species too (e.g. if forme changed but nickname pointed here)
                if get_base_species(initial_species)[-1] == get_base_species(normalized_species_lookup)[-1]:
                     return found_slot_id
        ...
        # 2b. Fuzzy Match - Try base species variants (e.g., mimikyubusted -> mimikyu)
        # Get all possible base forms of the lookup species
        ...
        # 2c. Substring/Nickname Match Fallback (New Logic)
        # Matches if normalized nickname is a substring of stored species, or vice versa
        # AND if it's a unique match (to avoid ambiguity)
        if identifier_name:
             norm_nick = normalize_species_name(identifier_name, strip_formes=False)
             possible_slots = []
             for slot_id, stored_normalized_species in slot_id_to_species.get(player, {}).items():
                  # Check if nickname is inside species name (e.g. "Swampert" in "swampertmega")
                  if norm_nick in stored_normalized_species or stored_normalized_species in norm_nick:
                       possible_slots.append(slot_id)
             
             if len(possible_slots) == 1:
                  match_slot = possible_slots[0]
                  nickname_to_slot_id.setdefault(player, {})[identifier_name] = match_slot
                  return match_slot

        # 3. Dynamic Slot Creation - For replays without |poke| lines
        # If species map is empty or this species hasn't been seen, create a new slot dynamically
        existing_slots = len(slot_id_to_species.get(player, {}))
        
        # FIX: Always allow at least 6 slots, regardless of declared teamsize (which might be wrong/missing).
        # Cap at 12 to handle weird tier shifts/illusion bugs without infinite growth.
        declared_size = current_teamsize.get(player, 6)
        if declared_size <= 0: declared_size = 6
        max_slots = max(6, declared_size)
        if max_slots > 12: max_slots = 12
        ...
        if existing_slots < max_slots:
            ...
            # print(f"DEBUG: Dynamically created {new_slot_id} for {player} with species '{normalized_species_lookup}'")
            return new_slot_id
    ...
    # FIX: Freeze state at turn start to prevent temporal data leakage.
    # Both players choose moves simultaneously; the state visible to each
    # player is only what existed at the START of the turn, not after
    # earlier-in-log moves have resolved.
    turn_start_state = None        # Frozen snapshot taken at |turn| line
    turn_start_last_move = {'p1': 'none', 'p2': 'none'}  # last_move from end of PREVIOUS turn
    ...
    for line_num, line in enumerate(lines):
        ...
        try:
            ...
            if event_type == 'turn':
                ...
                # FIX: Snapshot the state RIGHT NOW before any move events fire.
                # This is the "ground truth" state both players see when choosing moves.
                turn_start_state = copy.deepcopy(current_state)
                turn_start_last_move = dict(last_move_seen)  # Copy last_move from previous turn
            ...
            # --- Core State Updates (HP, Status, Boosts) --- (Unchanged)
                ...
                # Handles moves like Belly Drum that SET boosts to a specific value
                if len(parts) >= 5:
                    identifier, stat, amount_str, *_ = parts[2:]
                    player, _ = parse_pokemon_identifier(identifier)
                    if player:
                ...
                # Handles moves that clear a specific Pokemon's boosts (e.g., Clear Smog on target)
                if len(parts) >= 3:
                    identifier = parts[2]
                    player, _ = parse_pokemon_identifier(identifier)
                    if player:
                ...
                # Handles Haze - clears ALL boosts on the field
                for player in ['p1', 'p2']:
                ...
                # Handles Z-Moves/effects that clear only negative boosts
                if len(parts) >= 3:
                    identifier = parts[2]
                    player, _ = parse_pokemon_identifier(identifier)
                    if player:
                    ...
                    if player:
                        ...
                        if incoming_slot_id:
                            ...
                            if incoming_slot_id in current_state.get(player, {}):
                                ...
                                # NOTE: Tera state persists for the entire battle, do NOT reset here
                                # poke_state['terastallized'] and poke_state['tera_type'] are preserved
            ...
            # --- Side Condition / Hazard Updates --- (Unchanged, keys already normalized in flatten)
                ...
                if len(parts) >= 4:
                    ...
                    if affected_side_player and condition:
                        # Handle hazard removal (Defog, Rapid Spin, Court Change, etc.)
                        target_hazard_key = f'{affected_side_player}_hazards'
                        if condition in HAZARD_CONDITIONS and target_hazard_key in current_state['field']:
                            hazard_dict = current_state['field'][target_hazard_key]
                            if condition in hazard_dict:
                                hazard_dict[condition] = 0
                        # Handle non-hazard side condition removal
                        target_side_key = f'{affected_side_player}_side'
            ...
            # --- Forme Change Handler ---
                ...
                # Handles mid-battle forme changes (Mega Evolution, Aegislash, Zygarde, etc.)
                if len(parts) >= 4:
                    ...
                    if player:
                        ...
                        if slot_id and player in current_state and slot_id in current_state[player]:
                            ...
                            if poke_state:
                                ...
                                # Note: We don't update slot_id_to_species here because
                                # we want to keep the original species for lookup purposes
            ...
            if is_player_action and acting_player:
                # FIX: Use the turn_start_state snapshot instead of live current_state.
                # This ensures both P1 and P2 actions in the same turn see the same
                # pre-turn board (no temporal leak from resolved damage/effects).
                base_state = turn_start_state if turn_start_state is not None else current_state
                state_snapshot = copy.deepcopy(base_state)
                # last_move reflects the state at START of turn (i.e. what happened last turn)
                state_snapshot['last_move_p1'] = turn_start_last_move.get('p1', 'none')
                state_snapshot['last_move_p2'] = turn_start_last_move.get('p2', 'none')
                ...
                # Update LIVE current_state AFTER snapshotting (does not affect already-appended snapshot)
